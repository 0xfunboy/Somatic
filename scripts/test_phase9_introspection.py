...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        mind = root / "data" / "mind"
        write_json(mind / "internal_loop_state.json", {"last_prompt": "PROMPT_X", "last_parsed": {"mode": "grow"}, "last_memory_updates": []})
        write_json(mind / "bios_state.json", {"last_task": "run_growth_cycle", "last_result": "ok", "last_evidence": {"command": "python3 scripts/test_answer_finalizer.py"}})
        write_json(mind / "metabolic_state.json", {"mode": "recover", "stability": 0.31, "stress": 0.82, "self_integrity": 0.74, "growth_allowed": False, "recovery_required": True, "reasons": ["stress_above_max"]})
        write_json(mind / "reward_state.json", {"rolling_score": 0.12, "trend": 0.03, "last_kind": "test_pass", "last_value": 0.15, "count": 4})
        write_json(mind / "mutation_state.json", {"last_blockers": ["growth_not_allowed", "recovery_required"]})
        ...
        failures += check("last internal json reads parsed state", '"mode": "grow"' in router.execute("show your last internal DeepSeek JSON")["text"])
        ...
        failures += check("why not mutating reports blockers", "growth_not_allowed" in router.execute("why are you not mutating?")["text"])
