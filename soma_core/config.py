...
class SomaConfig:
    ...
    v10_internal_radio: bool = field(default_factory=lambda: _bool("SOMA_V10_INTERNAL_RADIO", True))
    broadcast_inner_prompts: bool = field(default_factory=lambda: _bool("SOMA_BROADCAST_INNER_PROMPTS", True))
    broadcast_inner_raw: bool = field(default_factory=lambda: _bool("SOMA_BROADCAST_INNER_RAW", False))
    broadcast_inner_evidence: bool = field(default_factory=lambda: _bool("SOMA_BROADCAST_INNER_EVIDENCE", True))
    internal_event_preview_chars: int = field(default_factory=lambda: int(_float("SOMA_INTERNAL_EVENT_PREVIEW_CHARS", 900.0)))
    internal_ledger_max_lines: int = field(default_factory=lambda: int(_float("SOMA_INTERNAL_LEDGER_MAX_LINES", 5000.0)))
    internal_llm_min_interval_sec: float = field(default_factory=lambda: _float("SOMA_INTERNAL_LLM_MIN_INTERVAL_SEC", 300.0))
    internal_llm_max_per_hour: int = field(default_factory=lambda: int(_float("SOMA_INTERNAL_LLM_MAX_PER_HOUR", 6.0)))
    internal_llm_skip_on_user_active_sec: float = field(default_factory=lambda: _float("SOMA_INTERNAL_LLM_SKIP_ON_USER_ACTIVE_SEC", 45.0))
    internal_llm_skip_in_resource_recovery: bool = field(default_factory=lambda: _bool("SOMA_INTERNAL_LLM_SKIP_IN_RESOURCE_RECOVERY", True))
    ui_speak_internal_default: bool = field(default_factory=lambda: _bool("SOMA_UI_SPEAK_INTERNAL_DEFAULT", False))
    ui_speak_replies_default: bool = field(default_factory=lambda: _bool("SOMA_UI_SPEAK_REPLIES_DEFAULT", False))
    ui_speak_max_queue: int = field(default_factory=lambda: int(_float("SOMA_UI_SPEAK_MAX_QUEUE", 5.0)))
